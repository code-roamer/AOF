...
def get_Laplace_from_pc(ori_pc):
    with torch.no_grad():
        point_mat = ori_pc.unsqueeze(1) - ori_pc.unsqueeze(0)
        A = torch.exp(-torch.sum(point_mat.square(), dim=2))

        mask = torch.zeros_like(A)
        idx = knn(ori_pc, 30)
        for i in range(idx.shape[0]):
            for jid in range(idx.shape[1]):
                j = idx[i, jid]
                mask[i, j] = 1
                mask[j, i] = 1
...
def attack():
    ...
    for pc, label, target in tqdm(test_loader):
        ...
        e, v = get_Laplace_from_pc(pc[0])
        
        at_pc = torch.tensor(best_pc[0]).to(pc)
        ori_pc = pc[0]
        py = []
        for i in range(0, 1024):
            xc = torch.sum(at_pc[:, 0]*v[:,i])
            yc = torch.sum(at_pc[:, 1]*v[:,i])
            zc = torch.sum(at_pc[:, 2]*v[:,i])

            ori_xc = torch.sum(ori_pc[:, 0]*v[:,i])
            ori_yc = torch.sum(ori_pc[:, 1]*v[:,i])
            ori_zc = torch.sum(ori_pc[:, 2]*v[:,i])
            py.append(torch.abs(ori_xc-xc).detach().cpu().item())
            print(f"{i}th component weight, ori:{ori_xc}, at:{xc}, error:{ori_xc-xc}")

        plt.plot(py)
        plt.savefig('/home/lbb/DLProjects/IF-Defense/baselines/visual/rate.png')
        plt.show()
        plt.close()
        # results
        num += success_num
...
if __name__ == "__main__":
    ...
    test_set = ModelNet40Attack(args.data_root, num_points=args.num_points,
                                normalize=True)
    test_sampler = DistributedSampler(test_set, shuffle=True)
