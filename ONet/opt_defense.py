...
def optimize_points(opt_points, z, c,
    ...
    for i in range(iterations + 1):
        # 1. occ = threshold
        occ_value = generator.model.decode(opt_points, z, c).logits
...
def get_save_name(path, train=args.train):
    ...
    save_name = 'onet_opt-' + save_name
    save_folder = path[:path.rindex(sub_path[-1])]
    save_folder = os.path.join(save_folder, 'ONet-Opt')
    ...
    for idx in tqdm.trange(0, len(pc), args.batch_size):
        ...
        with torch.no_grad():
            ...
            z = generator.model.get_z_from_prior(
                (len(batch_pc),), sample=generator.sample).cuda()
